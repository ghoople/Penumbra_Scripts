/*
 * Title: Black Box Demo, hard coded for black box specifics
...
void loop() {
  ...
  State_DemoButton = digitalRead(DemoButton);
  eject = false;
  ...
  if(State_UpButton == HIGH){
    ...
    motor.Move(-7840, MotorDriver::MOVE_TARGET_ABSOLUTE); // Commands the motor move (with no waiting step before commanding additional moves!)

  }
  else if(State_DownButton == HIGH){
    motor.VelMax(User_Speed); // Sets the maximum velocity for this move based on the potentiometer
    motor.Move(0, MotorDriver::MOVE_TARGET_ABSOLUTE); // Commands the motor move (with no waiting step before commanding additional moves!)
    //Serial.print("Moving down at user speed: ");
    //Serial.println(User_Speed);
  }
  else if(State_DemoButton == HIGH){
    //Serial.print("Entering demo mode");
    delay(1000);
    State_DemoButton = digitalRead(DemoButton);
    while(State_DemoButton == LOW){
      motor.VelMax(User_Speed); // Sets the maximum velocity for this move based on the potentiometer
      motor.Move(0, MotorDriver::MOVE_TARGET_ABSOLUTE);
      while (!motor.StepsComplete()) {
        State_DemoButton = digitalRead(DemoButton);
        Value_SpeedPot = analogRead(SpeedPot); 
        User_Speed = map(Value_SpeedPot,0,3380,500,velocityLimit);

        if(State_DemoButton == HIGH){
          motor.MoveStopDecel(4000);
          eject = true;
        }
      }
      if(eject == true){return;}
      motor.VelMax(User_Speed); // Sets the maximum velocity for this move based on the potentiometer
      motor.Move(-7840, MotorDriver::MOVE_TARGET_ABSOLUTE);
      while (!motor.StepsComplete()) {
        State_DemoButton = digitalRead(DemoButton);
        Value_SpeedPot = analogRead(SpeedPot); 
        User_Speed = map(Value_SpeedPot,0,3380,500,velocityLimit);
